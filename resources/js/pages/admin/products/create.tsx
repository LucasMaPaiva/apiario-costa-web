...
export default function AdminProductCreate() {
    ...
    const { handleCreateProduct } = useAdminProducts();
    ...
    const validate = () => {
        ...
        if (formData.stock === '' || Number(formData.stock) < 0) newErrors.stock = 'Informe o estoque inicial.';
        if (!mainImage) newErrors.mainImage = 'Selecione uma imagem de capa.';
        return newErrors;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await handleCreateProduct(formData, mainImage, carouselImages);
            navigate('/admin');
        } catch (error: any) {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20">
            <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h1 className="text-3xl font-bold italic text-text-primary mb-1">Novo Produto</h1>
                    <p className="text-sm text-text-secondary">Cadastre um novo item no catálogo da Apiário Costa.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-10">
                ...
                <div className="bg-surface p-12 rounded-[2.5rem] shadow-2xl border border-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                placeholder="Ex: Mel de Flor de Laranjeira"
                            />
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className={inputClass('slug', "w-full px-5 py-4 bg-bg-main/50 border border-border rounded-xl focus:outline-none focus:border-brand-mel focus:ring-4 focus:ring-brand-mel/10 transition-all font-medium text-sm text-text-secondary italic")}
                            />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                        ...
                        <div className="space-y-2">
                            <label className="block text-[10px] uppercase font-black text-text-secondary tracking-[0.15em] ml-1 flex items-center gap-2">
                                <Database size={12} /> Estoque Inicial *
                            </label>
                            <input
                                type="number"
                ...
                <div className="bg-surface p-12 rounded-[2.5rem] shadow-2xl border border-border">
                    ...
                    <div className="mb-10">
                        ...
                        <div className={`flex flex-col md:flex-row items-center gap-8 p-6 bg-bg-main/50 rounded-2xl border-2 border-dashed group hover:border-brand-mel/50 transition-colors ${errors.mainImage ? 'border-red-500' : 'border-border'}`}>
                            ...
                            <div className="flex-1 text-center md:text-left">
                                <p className="text-sm font-bold text-text-primary mb-1">Arraste uma imagem ou clique para selecionar</p>
                                ...
                                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                                    <label htmlFor="mainImage" className="cursor-pointer px-6 py-2.5 bg-brand-mel text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-brand-gold transition-all shadow-md shadow-brand-mel/10">Selecionar Imagem</label>
                                    {mainImagePreview && (
                    ...
                    <div>
                        <label className="block text-[10px] uppercase font-black text-text-secondary tracking-[0.15em] mb-4 ml-1">Outros ângulos (Carrossel)</label>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {carouselPreviews.map((preview, index) => (
                                <div key={index} className="relative aspect-square bg-bg-main rounded-xl overflow-hidden group border border-border">
                            ...
                            <label className="aspect-square bg-bg-main border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-brand-mel hover:bg-brand-mel/5 hover:text-brand-mel transition-all text-text-secondary group">
                                ...
                                <input type="file" multiple accept="image/*" className="hidden" onChange={handleCarouselImagesChange} />
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between bg-surface p-8 rounded-2xl shadow-xl border border-border sticky bottom-8 z-10">
                    <div className="hidden md:block">
                        <p className="text-xs font-bold text-text-secondary">Verifique todas as informações antes de salvar.</p>
                    </div>
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        >
                            <Save size={18} /> {loading ? 'Processando...' : 'Finalizar Cadastro'}
                        </button>
                    </div>
                </div>
            </form>
